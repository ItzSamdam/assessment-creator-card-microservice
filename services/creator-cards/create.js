...
const { generateSlugFromTitle, isSlugUnique, serializeCard } = require('@app/utils/common');
...
async function createCreatorCard(serviceData, options = {}) {
    ...
    try {
        ...
        if (data.access_code) {
            throwAppError(CreatorCardMessages.ACCESS_CODE_REQUIRED, ERROR_CODES.AC01);
        }
        ...
        if (accessType === 'public' && data.access_code) {
            throwAppError(CreatorCardMessages.ACCESS_CODE_NOT_ALLOWED, ERROR_CODES.AC05);
        }
        ...
        if (!slug) {
            ...
            if (!unique) {
                throwAppError(CreatorCardMessages.SLUG_TAKEN, ERROR_CODES.SL02);
            }
        }
        ...
        const card = await creatorCardRepository(cardData);
