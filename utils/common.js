...
const generateSlugFromTitle = async (title) => {
    ...
    let slug = baseSlug;
    let exists = await creatorCardRepository.findOne({ slug });
    let counter = 0;

    while (exists && counter < 10) {
        const suffix = Math.random().toString(36).substring(2, 8);
        slug = `${baseSlug.slice(0, 40)}-${suffix}`;
        exists = await creatorCardRepository.findOne({ slug });
        counter++;
    }
...
const isSlugUnique = async (slug, excludeId = null) => {
    ...
    const existing = await creatorCardRepository.findOne(query);
    return !existing;
};
